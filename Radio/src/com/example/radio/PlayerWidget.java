...
public class PlayerWidget extends AppWidgetProvider {
	...
	final String LOG_TAG = "myLogs";
	  ...
	  public void onEnabled(Context context) {
		...
		if (MediaPlayerServiceRunning(context)) {

			ComponentName service = new ComponentName(context,
					PlayerService.class);

			Intent intent = new Intent(PlayerService.SET_STATION)
					.setComponent(service);
			PendingIntent pIntentStop = PendingIntent.getService(context, 0,
					intent, 0);
			try {
				pIntentStop.send();
	  ...
	  private boolean MediaPlayerServiceRunning(Context context) {
